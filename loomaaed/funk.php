...
function kuva_puurid(){
	...
	$p= mysqli_query($connection, "SELECT DISTINCT(puur) as puur FROM 10162844_anaava_loomaaed order by puur asc");
	$puurid=array();
	while ($r=mysqli_fetch_assoc($p)){
		
		$l=mysqli_query($connection, "SELECT * FROM 10162844_anaava_loomaaed WHERE  puur=".mysqli_real_escape_string($connection, $r['puur']));
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			if($_POST["user"] == '' || $_POST["pass"] == ''){
				...
				if(empty($_POST["pass"]))
					...
					$sql = "SELECT id FROM 10162844_anaava_kylastajad WHERE username='$kasutaja' AND passw=sha1('$parool')";
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			if($_POST["nimi"] == '' || $_POST["puur"] == ''){
				...
				if(empty($_POST["puur"])) {
					...
					$sql = "INSERT INTO 10162844_anaava_kylastajad (nimi, puur, liik) VALUES ('$nimi', '$puur', '$liik')";
					$result = mysqli_query($connection, $sql);
